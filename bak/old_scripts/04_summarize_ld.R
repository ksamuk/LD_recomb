...
summarize_ld <- function(x, overwrite = FALSE){
  ...
  file_slugs <- gsub(".*data/sim_ld/", "", x) %>%
  strsplit(split = "/") %>% unlist

  root_folder <- paste0("data/sim_ld/summary/", file_slugs[1])
  dir.create(root_folder)

  file_slug <- paste0(root_folder, "/", file_slugs[2], "_", "ld_summary.txt")
...
ld_list <- lapply(list.files("data/sim_ld/summary", full.names = T, recursive = T), read.table, h = T)
