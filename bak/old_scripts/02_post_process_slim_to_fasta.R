...
write_fasta_out <- function(genome_id_sub, genome_df_in, file_name, blank_seq, seq_lenth = 100000){
  
  hap_df_sub <- genome_df_in %>%
    filter(genome_id == genome_id_sub)
  
  hap_df_sub$pos <- hap_df_sub$pos[(hap_df_sub$pos >= 0) & (hap_df_sub$pos <= seq_lenth)]
  ...
  # grab the population string from the genome_id
  pop <- gsub("\\:[0-9]*", "", genome_id_sub)
  file_name <- gsub("p_", paste0(pop, "_"), file_name)
...
slim_to_fasta <- function(slim_generation, slim_sim_prefix, slim_sim_run, mut_mat_file, seq_lenth = 100000, n_samples = 20){
  
  full_slim <- paste0(slim_sim_run, "/", slim_generation, "_full.txt.gz")
  fixed_slim <- paste0(slim_sim_run, "/", slim_generation, "_fixed.txt.gz")
  ...
  fasta_out_folder <- paste0("data/fasta_output/", gsub("data/slim_output/", "", slim_sim_prefix),"/", sim_num)
  dir.create(fasta_out_folder)
  
  # the fasta file to be writen
  out_file_name <- paste0(fasta_out_folder, "/", "p_",gen_num, ".fasta")
...
process_slim_output_to_fasta <- function(slim_sim_prefixes, mut_mat_file, n_cores = 1){
  ...
  for (i in slim_sim_prefixes){
    ...
    # output folders (create if non-existant)
    fasta_folder <- paste0("data/fasta_output", gsub("data/slim_output", "", i))
    dir.create(fasta_folder)
    ...
    for (j in slim_sim_runs){
      ...
      generations <- list.files(j, pattern = "full.txt.gz") %>%
        gsub(j, "", .) %>%
        gsub("/", "", .) %>%
        gsub("_full.txt.gz", "", .) %>%
        as.numeric %>%
      ...
      print(paste0("no files to process for ", j))
      }
...
process_slim_output_to_fasta(slim_sim_prefixes = slim_sim_prefixes, mut_mat_file = mut_mat_file, n_cores = 1)
